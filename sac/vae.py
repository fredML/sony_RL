...
class VAE(hk.Module): 
    ...
    def __call__(self, s, is_training):
        ...
        recons = self.decoder(h, is_training)
...
class ResNetVAE(hk.Module):
    def __init__(self, latent_dim, filter_sizes):
        ...
        super().__init__()
        self.latent_dim = latent_dim
        self.encoder = hm.ResNet50(include_top=False)
        self.decoder = Decoder_2(filter_sizes[::-1])
    ...
    def __call__(self, s, encoder_training, encoder_bn_training):
        ...
        h = hk.Flatten()(enc_output)
        latent = hk.Linear(self.latent_dim)(h)
